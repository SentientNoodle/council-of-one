...
begin
    ...
    set @sql = concat('create temporary table scoreboard as (select ID,Title,Content,(((length(Title) - length(replace(lower(Title),lower("',@lastTerm,'"),""))) / (length("',@lastTerm,'") * 0.5)) + ((length(Content) - length(replace(lower(Content),lower("',@lastTerm,'"),""))) / length("',@lastTerm,'"))');
    while @i<@n do
        select min(variable) from tmpSplit where variable > @lastTerm into @lastTerm;
        set @sql := concat(@sql,' + ((length(Title) - length(replace(lower(Title),lower("',@lastTerm,'"),""))) / (length("',@lastTerm,'") * 0.5)) + ((length(Content) - length(replace(lower(Content),lower("',@lastTerm,'"),""))) / length("',@lastTerm,'"))');
        set @i=@i+1;
    end while;
select @sql;
